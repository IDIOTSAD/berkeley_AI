...
class DiscreteDistribution(dict):
    ...
    def normalize(self):
        ...
        if tot < 0:	#음수인 경우, 양수로 전환시킴
            tot = -tot
        elif tot == 0:	#0인 경우, 1로 전환, 아무것도 수행하지 않음을 의미
            tot = 1
...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation, gameState):
        ...
        for pos in self.allPositions:	#모든 귀신이 움직일 수 있는 위치의 수만큼 반복
            self.beliefs[pos] = self.beliefs[pos] * self.getObservationProb(
                noisyDistance=observation,	#관측된 거리, 노이즈가 존재함.
    ...
    def elapseTime(self, gameState):
        ...
        """
        "*** YOUR CODE HERE ***"
        new_beleifs = DiscreteDistribution()	#새로운 믿음에 의한 확률을 새로 정의
        new_beleifs.normalize()	#새로 정의하였던 확률들을 정규화 시킴.
        for pos in self.allPositions:	#모든 귀신이 움직일 수 있는 위치의 수만큼 반복
            new_pos_dist = self.getPositionDistribution(gameState, pos)	#새로운 위치에 의한 거리의 확률들을 얻어옴.
            for new_pos in self.allPositions:	#모든 귀신이 움직일 수 있는 위치의 수만큼 반복
                new_beleifs[new_pos] += new_pos_dist[new_pos]*self.beliefs[pos]
				#새로운 믿음에 의한 확률을 갱신시킴, 갱신 시 이후의 이동분포(확률)와 기존의 확률을 곱함.
				#베이즈 추론(이론), 확률적 평균값으로 기대값을 계산함. 확률의 확률을 사용하여 추정함. P(b|a) * P(a) = P(a,b)
        new_beleifs.normalize()	#갱신을 통해서 새로운 확률을 정규화 시킴.

        self.beliefs = new_beleifs	#위에서 구한 확률을 갱신시킴.
